/*
  ...
  1. Changes
    - Optimize auth function calls in RLS policies to prevent per-row evaluation
    - Consolidate multiple permissive policies to reduce query overhead
    - Preserve all existing functionality and authorization logic
  
  2. Benefits
    - Faster queries - Auth functions evaluated once per query instead of per row
    - Reduced CPU usage - Fewer policy evaluations per database operation
    - Better scalability - Performance stays consistent as data grows
    
  3. Rollback Instructions
    - To rollback, restore original policies from the comments in this migration
*/
...
-- For anonymous management
CREATE POLICY "Anyone can manage collages"
  ON collages FOR UPDATE OR DELETE
  TO public
  USING (true);
